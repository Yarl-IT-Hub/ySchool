...
if(!$_REQUEST['modfunc'])
{
	...
	if($_REQUEST['id'])
	{
		...
		DrawHeaderHome($title,$delete_button.SubmitButton('Save','','class=btn_medium onclick="formcheck_student_contactField_F1();"')); //'<INPUT type=submit value=Save>');
	...
	elseif($_REQUEST['category_id'])
	{
		...
		echo " method=POST>";
		DrawHeaderHome($title,$delete_button.SubmitButton('Save','','class=btn_medium onclick="formcheck_student_contactField_F2();"')); //'<INPUT type=submit value=Save>');
		$header .= '<TABLE cellpadding=3 width=100%>';
		$header .= '<TR>';
